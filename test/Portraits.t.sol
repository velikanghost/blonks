...
contract PortraitsTest is Test {
    ...
    function test_InitialState() public view {
        assertEq(portraits.name(), "Temporal ASCII Portraits");
        assertEq(portraits.symbol(), "TAP");
        assertEq(portraits.owner(), owner);
    }
    ...
    function test_Mint() public {
        ...
        vm.startPrank(user1);
        portraits.mint{value: 0.01 ether}();
        vm.stopPrank();

        assertEq(portraits.balanceOf(user1), 1);
        assertEq(portraits.ownerOf(1), user1);
        assertEq(portraits.originalMinter(1), user1);
    }
    ...
    function test_RevertWhen_MintUnderpriced() public {
        ...
        portraits.mint{value: 0.009 ether}();
        vm.stopPrank();
    }
    ...
    function test_TokenURIContent() public {
        ...
        string memory uri = portraits.tokenURI(1);
    ...
    function test_PortraitEvolution() public {
        ...
        // Get initial URI
        string memory uri1 = portraits.tokenURI(1);
        
        // Move forward 100 blocks
        vm.roll(block.number + 100);
        
        // Get new URI
        string memory uri2 = portraits.tokenURI(1);
        
        // URIs should be different due to temporal variation
        assertTrue(keccak256(bytes(uri1)) != keccak256(bytes(uri2)), "Portrait should evolve over time");
    }
    ...
    function test_ConsistentPortraits() public {
        ...
        assertEq(keccak256(bytes(uri1)), keccak256(bytes(uri2)), "Portrait should be deterministic");
    }
    ...
    function test_Withdraw() public {
        ...
        // Verify withdrawal
        assertEq(address(portraits).balance, 0);
