...
library ASCIIPortraitLib {
    ...
    string constant SHADE_CHARS = " .'`^\",:;Il!i><~+_-?][}{1)(|\\/tfjrxnuvczXYUJCLQ0OZmwqpdbkhao*#MW&8%B@$";
     ...
     */
    function _random(bytes32 seed, uint256 nonce) internal pure returns (uint256) {
     ...
     */
    function _getShade(uint256 intensity) internal pure returns (string memory) {
     ...
     */
    function _generateBaseFeatures(address subject) internal pure returns (bytes32) {
     ...
     */
    function _getTemporalVariation(uint256 blockNum) internal pure returns (uint256) {
        return (blockNum / 100) % 8; // 8 different variations
    }

    /**
     * @dev Get face template intensity for a given position
     */
    function _getFaceIntensity(uint256 x, uint256 y, bytes32 seed, uint256 variation) internal pure returns (uint256) {
     ...
     */
    function _assemblePortrait(Portrait memory portrait) internal pure returns (string memory) {
